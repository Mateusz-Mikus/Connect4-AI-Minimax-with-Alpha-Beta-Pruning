...
def main():
    ...
    board_area = pygame.Rect(0,150,700,600)
    ...
    algorithm_array.append((algorithm_minimax, "MINIMAX"))
    algorithm_array.append((algorithm_minimax_alfa_beta, "MINIMAX_ALPHA_BETA"))
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if turn == 1 and board_area.collidepoint(event.pos):
                    posX = event.pos[0]
                    col = int (posX // 100)
        ...
        if turn == 2 and winner is None:
            ...
            if valid_loc:
                ...
                if algorithm_choice == "MINIMAX":
                    best_col = Ai.get_best_move(myBoard, ai_depth, False)
                else:
                    best_col = Ai.get_best_move(myBoard, ai_depth, True)
                end_time = time.time()
                ...
                if Board.check_win(myBoard, turn):
                    winner = turn
                    show_message = True
                elif Ai.is_game_finished(myBoard):
                    winner = 0
                    show_message = True
                turn = 1
            else:
